...
class $modify(GJOptionsLayer) {
	void goToPage(int page) {
		...
		} else if (auto node = fooBar->m_mainLayer->getChildByType<CCLabelBMFont>(0)) geode::Notification::create(fmt::format("node: {}, string: {}, fntFile: {}", node != nullptr, node->getString(), node->getFntFile()), 10.f)->show();
		else geode::Notification::create("I FUCKING HATE ROBTOP, SCREENSHOT THIS", 10.f)->show();
